...
- **Project Learnings** - Self-learning for agents working inside a project (a repo, folder, or vault). At the start of a task the agent retrieves what earlier sessions learned about operating in the project; at the end it reflects and saves the durable insights — where things live, traps, conventions, decisions with their rationale, workflows, open questions — to an opt-in, append-only knowledge graph (`learn-<project>`) in your InfraNodus account that any client and any machine can query. Uses the [InfraNodus MCP server](https://infranodus.com/mcp) `get_project_learnings` / `add_project_learnings` / `enable_project_learnings` tools. Nothing is written without your explicit per-project opt-in and a review of each batch; only project knowledge is kept, never anything about you.
