...
3. **Ontology** — an AI-condensed ontology graph (`onto-<project>`: entities
...
Full + Digest are two script runs (scopes are independent files that share
the manifest). Note the choices in your reply before building. Filtered scans get their own suffixed scope files and graphs
