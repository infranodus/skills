...
3. **Principles digest + structural feedback** — a digest YOU write: read
   ...
   → `repo2statements.py . <scope flags> --principles` (see Step 2)
...
**Principles vs ontology** — both are LLM condensations, but they differ:
principles = prose statements about how things work, written by you from
...
by "how does X connect to Y" queries. Offer principles when the user wants
...
### Principles digest (`--principles`) — you write this scope
...
- "What are the rules / how is X supposed to be done / why is it done this
  way" → `retrieve_from_knowledge_base` on `principles` if it exists,
  else `docs`.
